...
`list_law_updates` は、更新件数が多い日でも通常の応答を簡潔に保つため、既定では公式順序の先頭 50 件を返します。`totalCount`、`returnedCount`、`omittedCount` および `truncated` で省略の有無と件数を確認でき、必要な場合は `limit` に 1 以上 512 以下を指定できます。
