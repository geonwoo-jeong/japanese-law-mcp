...
必須の事件番号、裁判年月日、裁判所若しくは法廷および入力と同じ詳細 URL を識別できない場合は `invalid_source_response` とする。詳細画面ではない構造へ変更された場合は `source_contract_changed` とする。同じ必須ラベルが異なる値で複数回現れる場合は `invalid_source_response` とする。

script を実行せず、HTML から参照される PDF、画像、stylesheet その他の resource を取得しない。

## 項目対応

詳細 path の分類を `publicationCategory` とし、画面のカテゴリー見出しを `sourceCategoryLabel` として保持する。その他は次のラベルを使用する。
...
支部を独立したラベルで確認できる場合だけ `summary.branchName` を設定する。法廷名または裁判所名の文字列から支部若しくは部を切り出さない。

ラベルがない値を、表示順または別カテゴリーの欄から推測しない。知的財産事件だけのラベルは `SOT-MODEL-021` に従い初期版の共通詳細へ混入させない。
...
PDF link は `SOT-IF-044` と同じ `JudicialDocumentLink` へ対応させる。PDF link だけの `要旨` を `summaryText` へ複製しない。
...
最高裁判所と知的財産事件を含む複数カテゴリー、ラベルの差異、欠落可能項目、重複ラベル、原審日付、複数行文字列、PDF link だけの要旨、404、入力 ref の改変、resource budget および検索からの往復を fixture で確認する。
