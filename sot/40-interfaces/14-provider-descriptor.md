...
| `verifiedAt` | date | はい | 宣言したすべての capability と operation について現在の契約を再確認した日付のうち最も古い日 |
...
`verifiedAt` は、宣言した capability またはその配下の operation の一部だけを新しく確認した日付で上書きしない。古い parser、mapping または取得条件が残る場合は、その最も古い再確認日を保持し、未確認の契約を新しい日付で隠さない。

parser contract、mapping、取得位置の解釈、設定境界または継続トークンの検証条件を変更した場合は、互換性が保たれる修正でも `adapterContractVersion` を少なくとも patch 更新する。
