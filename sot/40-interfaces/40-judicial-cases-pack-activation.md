...
- 状態: 廃止
- 廃止理由: `judicial-cases` に依存する `judicial-citations` の有効化、公開ツールおよび route の組合せを表せないため
- 後継: [SOT-IF-067: `judicial-cases` と `judicial-citations` の有効化](67-judicial-citations-pack-activation.md)

## 規定

本規定を `SOT-IF-067` に置き換えた。以下は廃止時点の履歴であり、現行の公開契約には適用しない。
