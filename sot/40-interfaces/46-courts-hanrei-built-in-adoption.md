...
- 状態: 廃止
- 廃止理由: 判例引用追跡に必要な HTML 候補検索 binding、PDF provider および条件付き route を表せないため
- 後継: [SOT-IF-074: 判例引用追跡の組込み採用](74-courts-hanrei-citation-built-in-adoption.md)

## 規定

本規定を `SOT-IF-074` に置き換えた。以下は廃止時点の履歴であり、現行の組込み契約には適用しない。
