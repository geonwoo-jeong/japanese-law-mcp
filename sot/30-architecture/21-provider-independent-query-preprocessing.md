...
共通前処理は、Unicode の比較用正規化、日本語の形態素解析、辞書語の抽出、編集距離、候補の一意性判定、および原文位置を持つ一般検索語の抽出を提供する。法令、裁判例その他の能力に固有の語彙、識別子、正式名称および採用する別名は共通前処理へ埋め込まず、能力別プロファイルまたは辞書から渡す。

一つの照会文に対する Kagome の解析は一回だけ行い、その token 列を辞書出現、誤記候補および形態素検索語で共有する。引用句は同じ前処理呼出しの中で原文の対応する区切りから抽出する。query profile は位置付きの前処理結果を利用し、原文を別の tokenizer、正規表現または独自の規則で再解析して検索語を補わない。
