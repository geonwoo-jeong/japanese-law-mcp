...
所有範囲、閉じた `category/value` および version 連動を定義する。
...
cue の構文 role は `SOT-MODEL-029` を定義元とし、本規定で重複して定義しない。
...
`SOT-MODEL-025` と `SOT-MODEL-029` に従う。

## version と変更

positive cue の `category`、`value`、`syntaxRole`、所有 profile または
同じ profile 内の意味境界を変更する場合は、同じ変更で少なくとも次を更新する。

- 対象 profile の `cues.json`
- 対象 profile の `cueSetVersion` と profile version
- 採用済み profile set version
- 必要な loader test、profile fixture および architecture test
- 公開既定の観測結果が変わる場合は `SOT-ENG-024` が要求する corpus、
  baseline および検索例カタログ

共通 loader、schema version または profile 間の衝突検査を変える場合は
`SOT-ENG-030` の version 規則に従う。target/resource の候補化、複数主題分離、
候補保持または公開 decision まで変わる場合は、関係する model と architecture
SOT を同じ変更で更新する。
