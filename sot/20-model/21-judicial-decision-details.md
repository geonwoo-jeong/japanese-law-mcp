...
| `holdingText` | string | いいえ | 詳細ページに文字として掲載された判示事項 |
| `summaryText` | string | いいえ | 詳細ページに文字として掲載された裁判要旨 |
...
- 知的財産事件だけが持つ当事者、権利種別、訴訟類型、発明等の名称、主な争点および上告情報は、共通項目へ意味を縮約せず初期版の公開モデルへ含めない。
- JSON はこの表の camelCase 項目名を使用し、省略可能な値がない項目は `SOT-MODEL-009` に従って省略する。

## 確認

共通概要の保持、詳細文字列、原審日付、欠落可能項目、PDF リンクだけの場合の文字列省略および JSON 表現を単体テストで確認する。
