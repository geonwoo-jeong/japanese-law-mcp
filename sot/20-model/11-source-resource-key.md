...
公式識別子がない HTML またはファイルでは、情報源ごとの SOT が canonical official URL components を識別子として採用できる。components には path と、同じ資源の識別に必要な場合だけ選択した query parameter または fragment を含められる。

canonical official URL components は、資格情報、署名、セッション ID、広告識別子、追跡値その他の一時的な値を含めない。query parameter の順序、重複 parameter の並び順、大文字小文字または不要な区切りの違いで別資源にならない場合は、情報源ごとの SOT が canonicalization 規則を定義し、契約テストで検証する。

名称、題名、住所または本文の類似性から識別子を生成しない。
