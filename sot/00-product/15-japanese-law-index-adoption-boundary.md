# SOT-PROD-015: 日本法令索引の採用保留境界

- 状態: 有効

## 規定

[日本法令索引](https://hourei.ndl.go.jp/help.html)は、2026年8月24日時点では
Japanese Law MCP の構造化 provider、capability、provider route または公開 MCP
tool として採用しない。

現在の製品で許可する利用は、次に限る。

- `SOT-ENG-022` に従う補足法令名辞書の根拠資料
- 利用者または保守者が公式画面上で人手により確認した参照先 URL
- 将来の採用可否を判断するための一次資料調査

HTML 画面、未公開 endpoint、旧 API 連携データまたは観測した画面挙動を前提に、
機械取得 provider を実装しない。画面上で観測できる query parameter や内部 identifier
から詳細 URL を自動生成しない。

## 公式に確認できる収録範囲

日本法令索引のヘルプと国立国会図書館の API 一覧から、少なくとも次の
資源が人手検索向けに公開されていることを確認できる。

- 公布された法令の索引情報
- 法令沿革
- 被改正法令
- 帝国議会・国会に提出された法案と条約承認案件の索引情報
- 法案の審議経過

ただし、これらは現時点では一つの機械取得契約で公開されていない。したがって、
法令、法案、沿革、被改正法令および審議経過を一つの provider に平坦化して採用
しない。

## e-Gov 法令コアとの対応可能範囲

日本法令索引のヘルプでは、公布済み法令と公布に至った法案について法令番号と
公布年月日を検索対象に含めること、および法案について提出回次、提出番号、
成立・承認年月日、成立・承認回次等を検索対象に含めることを示している。

一方、e-Gov 法令 API Version 2 の公式 OpenAPI では、`GET /laws` が公布済み
現行法令の `law_info.law_id`、`law_info.law_num` および
`law_info.promulgation_date` を返し、`GET /law_data/{law_id_or_num_or_revision_id}`
が法令番号または法令 ID のどちらでも現行法令を取得できるとしている。

このため、公布済み現行法令に限れば、少なくとも法令番号を主キーとし、必要に応じて
公布年月日と法令名を補助条件に使う crosswalk 候補を設計できる。ただし、これは
現時点で公式対応表があることを意味しない。その crosswalk は e-Gov 側の現行法令
集合に存在する範囲に限られ、次を保証しない。

- 日本法令索引だけに存在する廃止法令、旧法令または沿革項目
- 法案、条約承認案件、審議経過と e-Gov lawId の 1 対 1 対応
- 日本法令索引の詳細画面 URL に見える query parameter を安定 identifier として使うこと

2026年8月24日時点で確認できる一次資料には、日本法令索引自身の stable identifier
体系、機械取得 contract、または e-Gov lawId との公式対応表は見当たらない。
したがって、将来採用するとしても、最初の候補は「公布済み現行法令の参照補助」に
限り、法案や審議経過を同じ識別子体系へ統合する前提を置かない。

## 保留理由

国立国会図書館の[API 一覧](https://www.ndl.go.jp/use/api)には、日本法令索引の専用
API が掲載されていない。国立国会図書館サーチの
[現行 API 連携先一覧](https://ndlsearch.ndl.go.jp/file/help/api/specifications/ndlsearch_api_ap1_20260624.pdf)
にも、日本法令索引は提供中の連携先として掲載されていない。さらに、
[2024年1月の変更告知](https://ndlsearch.ndl.go.jp/news/renkei_20240105)では、
日本法令索引および日本法令索引〔明治前期編〕との API 連携終了が明示されている。

日本法令索引の[ヘルプ](https://hourei.ndl.go.jp/help.html)では、同サービスが法令と
法案の索引情報を提供し、法令データは基本的に 2月、5月、8月、11月に更新すると
示している。一方で、利用条件として、短時間の大量アクセスを避けること、
機械的アクセスでは多重 request を避け、各 request の間隔を数秒程度空けることが
求められている。さらに、法令データは外部機関である第一法規株式会社から提供を
受け、データベース自体の著作権は国立国会図書館に帰属すると示している。利用に
あたっては、[国立国会図書館ウェブサイトのコンテンツ利用規約](https://www.ndl.go.jp/sitepolicy/terms)
も確認が必要である。同規約は
[公共データ利用規約（第1.0版）](https://www.digital.go.jp/resources/open_data/public_data_license_v1.0)
に準拠した利用条件を示す一方、第三者の権利があるコンテンツや外部データベース等
との API 連携で取得したコンテンツについては、その提供元の利用条件確認を利用者に
求めている。日本法令索引のヘルプも、法令データが第一法規株式会社から提供され、
データベース自体の著作権は国立国会図書館に帰属すると示している。したがって、
一般的な転載条件だけを根拠に、取得結果の cache、snapshot、派生辞書化、再配布
または商用利用を含む構造化 provider の採用可否を決めない。

同ヘルプは機械的アクセス自体を一律に禁止しているわけではない。しかし、公開して
いるのは HTML 画面へのアクセス上の注意であり、versioned schema、pagination、
差分取得、安定 identifier、欠落表現または互換性方針を備えた構造化取得契約ではない。
検索条件として法令番号、法案の国会回次、法案種別および提出番号等を扱えることは
確認できるが、それらと e-Gov 法令 API の identifier を対応付ける公式 crosswalk は
示されていない。画面上で観測できる内部 identifier も、外部利用者向けの安定契約とは
扱わない。

また、同ヘルプが示す収録範囲には時期と法令種別による制約がある。したがって、将来
構造化取得手段が公開された場合でも、日本法令索引の結果が法令沿革または法案情報を
網羅するとは仮定せず、収録範囲、欠落および更新基準を typed metadata として扱う。

`SOT-PROD-007` は、同じ意味と範囲を持つ公式 API または構造化配布物がない場合、
同規定の条件を満たす公式 HTML の自動取得を一般的な選択肢として認めている。
それに対し、この SOT は日本法令索引に固有の追加判断として、安定 identifier、
構造化取得契約および再利用条件の境界を一次資料で確認できない間は HTML provider
を採用しない。これは HTML を技術的に取得できるかどうかではなく、製品が依存する
契約と権利の境界を検証できるかどうかによる判断である。

## 現時点で採用しない範囲

次は採用しない。

- 日本法令索引の検索結果 HTML を解析する provider
- ブラウザ挙動に依存する headless scraping
- 国立国会図書館サーチの旧連携 ID、旧 identifier または過去 dump の再利用
- 全件収集、定期クロール、バックグラウンド更新、稼働監視
- 日本法令索引だけを根拠にした法令本文、法案本文または審議経過本文の生成
- 日本法令索引の法案・審議経過を e-Gov lawId と同一 identifier 空間へ正規化すること

日本法令索引の索引情報は、法令本文、法案本文、会議録本文または法的結論と同一視
しない。

## 将来の採用条件

将来、日本法令索引を構造化 provider として採用するには、少なくとも次を同じ変更
単位で満たす。

1. この SOT が日本法令索引に固有の追加条件として、公式に公開された構造化 API
   または downloadable dataset が存在し、versioned schema と現在の利用条件を
   一次資料で確認できることを要求する。この条件を `SOT-PROD-007` のすべての
   HTML 情報源へ一般化しない。
2. 法令索引、法案索引、法令沿革、被改正法令、審議経過のうち、どの資源をどの
   型付き capability で扱うかを新しい SOT で分離して定義できる。
3. 安定した識別子、pagination、継続取得、欠落、更新単位および日付の意味を
   interface SOT に定義できる。e-Gov 法令との対応を提供する場合は、法令番号等に
   よる公式または検証可能な crosswalk と不一致時の扱いも定義する。
4. `SOT-PROD-007` に従い、短時間大量アクセスの回避、多重 request の禁止、
   数秒程度の間隔という公開条件に反しない取得方式を、リクエスト内だけで実装できる。
5. 取得結果の一時保持、cache、snapshot、派生物、再配布および商用利用について、
   第三者の権利と提供元条件を含む適用条件を一次資料で確認できる。
6. 利用シナリオ、情報モデル、source mapping、built-in adoption および検証方法を
   pack 単位で採用してから実装へ進める。
7. 公布済み現行法令との crosswalk を採用する場合は、法令番号、公布年月日、
   法令名の照合条件、不一致時の扱いおよび適用対象外資源を interface SOT に固定する。

## 確認

次を確認する。

- この SOT から API 一覧、現行 API 連携先一覧、連携終了告知、日本法令索引ヘルプ
  および利用規約へ到達できること
- この SOT から e-Gov 法令 API Version 2 の OpenAPI と `SOT-IF-004` へ到達でき、
  `law_info.law_id`、`law_info.law_num` および `law_info.promulgation_date` を
  crosswalk 候補の根拠として確認できること
- provider 登録、route catalog および公開 tool 定義を検索し、現在の実装に
  `hourei.ndl.go.jp` を source provider とする binding、route または公開 tool が
  存在しないこと
- 補足法令名辞書の根拠として使う場合でも、`SOT-ENG-022` の範囲を超えて
  provider 採用へ拡張しないこと。各補足 entry が個別の公式 URL と e-Gov 法令 API
  Version 2 の公式スナップショットに存在する法令 ID を持つこと

## 関連

- [SOT-PROD-003: 法情報の採用基準](03-legal-source-eligibility.md)
- [SOT-PROD-004: 機能の採用条件](04-feature-adoption.md)
- [SOT-PROD-007: 情報源取得方式の選択](07-source-acquisition-policy.md)
- [SOT-PROD-014: 立法過程拡張パックの国会発言検索](14-legislative-history-extension-pack.md)
- [SOT-ENG-022: 法令名検索辞書](../50-engineering/22-law-name-search-lexicon.md)
- [SOT-IF-004: e-Gov 法令 API Version 2](../40-interfaces/04-source-egov-law-api-v2.md)
- [SOT-IF-054: e-Gov 法令名検索マッピング v3](../40-interfaces/54-egov-law-search-mapping-v3.md)
- [e-Gov 法令 API Version 2 OpenAPI](https://laws.e-gov.go.jp/api/2/swagger-ui/lawapi-v2.yaml)
