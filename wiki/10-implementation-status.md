...
- [SOT-SCN-012](../sot/10-scenarios/12-list-law-revisions.md)、[SOT-MODEL-032](../sot/20-model/32-law-revision.md)、[SOT-IF-055](../sot/40-interfaces/55-law-revision-list-capability.md)、[SOT-IF-056](../sot/40-interfaces/56-mcp-list-law-revisions.md) および [SOT-IF-057](../sot/40-interfaces/57-egov-law-revision-mapping-and-adoption.md) に従う、e-Gov 法令 API Version 2 の完全な改正履歴取得、列挙値と日付の共通モデル正規化、完全件数、出典、組込み primary route および公開 `list_law_revisions`
- [SOT-IF-004](../sot/40-interfaces/04-source-egov-law-api-v2.md) に従う、一つの MCP リクエスト内だけで共有して別リクエストと分離する開始時刻管理、全 HTTP attempt の実開始間隔一秒、有効な `Retry-After` の一秒下限、request deadline を越える待機の禁止、五 operation が共有する最大三回の再試行、ならびに `egov-http` 同時実行枠と交差 operation の `source_busy` 検証を前記 `conformanceTarget` から実行する接続
...
- [SOT-MODEL-031](../sot/20-model/31-shared-terminal-sequence.md)、
  [SOT-ARCH-021](../sot/30-architecture/21-provider-independent-query-preprocessing.md)
  および
  [SOT-ARCH-025](../sot/30-architecture/25-unified-query-multi-topic-separation.md)
  に従う、法令名・法概念・一般検索語の既存 span だけから閉じた separator、
  末尾接続、節末 `direct_task` relation および一意な最大列を一回確認する
  `SharedTerminalSequence`、深い複製を返す三 accessor、二百五十六 topic・
