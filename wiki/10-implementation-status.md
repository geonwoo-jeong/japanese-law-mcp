...
- [SOT-IF-018](../sot/40-interfaces/18-provider-configuration.md)、[SOT-IF-039](../sot/40-interfaces/39-configuration-sources-and-precedence-v2.md)、[SOT-IF-026](../sot/40-interfaces/26-provider-routing-configuration.md) および [SOT-IF-040](../sot/40-interfaces/40-judicial-cases-pack-activation.md) に従う、既存設定項目、`providers` と `providerRoutes`、key 単位の atomic な上書き、明示的な空 namespace、credential 環境変数参照の構造検証、設定ファイルだけから読み込む既定無効の `extensionPacks.judicial-cases.enabled`、有効な組込み provider と能力別 route の起動前照合、二つの裁判例ツールの原子的な公開、および不完全な構成の transport 起動前 fail-closed
...
- 現時点で、採用済みの `judicial-cases` 拡張パックに関する未実装項目はない。

引数を指定しないルートコマンドは stdio MCP サーバーを起動する。無設定時の公開ツールは `search_laws`、`get_law`、`get_article`、`search_law_content` および `list_law_updates` の五つである。`streamable-http` を指定した場合は、検証済みの loopback 待受先で `/mcp` を提供し、stdio と同じ公開ツール集合を使用する。
