...
現在は e-Gov 法令 API Version 2 の四つと、e-Gov 法令 API Version 1 の `law.update.list@1` の、合計五つの組込み primary route を起動時に構成する。公開 MCP ツールは従来の四つのままである。利用者指定の provider routing と credential environment reference はまだ扱わない。設計上の現在の定義元は [SOT-IF-026](../sot/40-interfaces/26-provider-routing-configuration.md)、[SOT-IF-029](../sot/40-interfaces/29-local-runtime-configuration.md) および [SOT-IF-037](../sot/40-interfaces/37-egov-v1-built-in-adoption.md) であり、設定入力の実装開始後に残る差分を解消する。
