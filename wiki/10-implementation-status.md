...
- [SOT-IF-004](../sot/40-interfaces/04-source-egov-law-api-v2.md) に従う、一つの MCP リクエスト内だけで共有して別リクエストと分離する開始時刻管理、全 HTTP attempt の実開始間隔一秒、有効な `Retry-After` の一秒下限、request deadline を越える待機の禁止、四 operation が共有する最大三回の再試行、ならびに `egov-http` 同時実行枠と交差 operation の `source_busy` 検証を前記 `conformanceTarget` から実行する接続
