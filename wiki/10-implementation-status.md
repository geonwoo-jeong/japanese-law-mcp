...
- `judicial-cases` のうち、既存の法令 route を必須のまま維持する任意の `judicial-decision.*` binding・route と、[SOT-IF-043](../sot/40-interfaces/43-source-courts-hanrei-html.md) から [SOT-IF-046](../sot/40-interfaces/46-courts-hanrei-built-in-adoption.md) までに従う `courts-hanrei-html` の検索・詳細 adapter、fixture、binding および条件付き route は未実装である。
