...
- [SOT-IF-018](../sot/40-interfaces/18-provider-configuration.md)、[SOT-IF-039](../sot/40-interfaces/39-configuration-sources-and-precedence-v2.md) および [SOT-IF-026](../sot/40-interfaces/26-provider-routing-configuration.md) のうち、既存設定項目、`providers` と `providerRoutes`、key 単位の atomic な上書き、明示的な空 namespace、credential 環境変数参照の構造検証、ならびに有効な組込み provider と能力別 route の起動前照合
...
- [SOT-IF-043](../sot/40-interfaces/43-source-courts-hanrei-html.md) と [SOT-IF-044](../sot/40-interfaces/44-courts-hanrei-search-mapping.md) に従う、固定 HTTPS origin、共有同時実行枠、HTML 資源予算、安全な DOM 解析、六カテゴリー・和暦・公式 PDF・出典の共通モデル対応および fixture を備え、runtime からは未到達の `courts-hanrei-html` `judicial-decision.search@1` planned adapter
...
- `judicial-cases` のうち、[SOT-IF-043](../sot/40-interfaces/43-source-courts-hanrei-html.md)、[SOT-IF-045](../sot/40-interfaces/45-courts-hanrei-read-mapping.md) および [SOT-IF-046](../sot/40-interfaces/46-courts-hanrei-built-in-adoption.md) に従う `courts-hanrei-html` の詳細 adapter、fixture、二能力の binding factory および条件付き route は未実装である。検索 adapter も activation までは runtime から到達しない。
- [SOT-IF-039](../sot/40-interfaces/39-configuration-sources-and-precedence-v2.md) と [SOT-IF-040](../sot/40-interfaces/40-judicial-cases-pack-activation.md) の `extensionPacks` 設定、および [SOT-IF-047](../sot/40-interfaces/47-mcp-search-judicial-cases.md) と [SOT-IF-048](../sot/40-interfaces/48-mcp-get-judicial-case.md) の二つの MCP ツールと composition root への公開接続は未実装である。
