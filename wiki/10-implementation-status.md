...
- [SOT-IF-043](../sot/40-interfaces/43-source-courts-hanrei-html.md)、[SOT-IF-044](../sot/40-interfaces/44-courts-hanrei-search-mapping.md) および [SOT-IF-045](../sot/40-interfaces/45-courts-hanrei-read-mapping.md) に従う、固定 HTTPS origin、共有同時実行枠、HTML 資源予算、安全な DOM 解析、六カテゴリー・和暦・公式 PDF・出典の共通モデル対応、検索から詳細への参照往復および fixture を備えた `courts-hanrei-html` `judicial-decision.search@1` と `judicial-decision.read@1` adapter。両 adapter は runtime からは未到達である
...
- `judicial-cases` のうち、[SOT-IF-046](../sot/40-interfaces/46-courts-hanrei-built-in-adoption.md) に従う `courts-hanrei-html` の二能力 binding factory、条件付き route および runtime への接続は未実装である。検索 adapter と詳細 adapter は activation までは runtime から到達しない。
- [SOT-IF-040](../sot/40-interfaces/40-judicial-cases-pack-activation.md) の有効時の一括構成、および [SOT-IF-047](../sot/40-interfaces/47-mcp-search-judicial-cases.md) と [SOT-IF-048](../sot/40-interfaces/48-mcp-get-judicial-case.md) の二つの MCP ツールと composition root への公開接続は未実装である。
