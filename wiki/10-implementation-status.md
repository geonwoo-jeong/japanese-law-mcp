...
- [SOT-PROD-013](../sot/00-product/13-law-version-comparison.md)、[SOT-MODEL-033](../sot/20-model/33-law-version-comparison.md)、[SOT-IF-058](../sot/40-interfaces/58-law-version-compare-capability.md)、[SOT-IF-059](../sot/40-interfaces/59-mcp-compare-law-versions.md) および [SOT-IF-060](../sot/40-interfaces/60-egov-law-version-comparison-mapping-and-adoption.md) に従う、指定した二つの法令版から本則と原始附則の条だけを抽出し、追加・削除・位置・文字列・内部構造の変更を決定的に分類する e-Gov 専用 adapter、資源上限、出典付き加工情報、組込み primary route および公開 `compare_law_versions`
- [SOT-IF-004](../sot/40-interfaces/04-source-egov-law-api-v2.md) に従う、一つの MCP リクエスト内だけで共有して別リクエストと分離する開始時刻管理、全 HTTP attempt の実開始間隔一秒、有効な `Retry-After` の一秒下限、request deadline を越える待機の禁止、六 operation が共有する最大三回の再試行、ならびに `egov-http` 同時実行枠と交差 operation の `source_busy` 検証を前記 `conformanceTarget` から実行する接続
