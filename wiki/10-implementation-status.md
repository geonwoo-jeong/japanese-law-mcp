...
- [SOT-IF-018](../sot/40-interfaces/18-provider-configuration.md)、[SOT-IF-039](../sot/40-interfaces/39-configuration-sources-and-precedence-v2.md)、[SOT-IF-026](../sot/40-interfaces/26-provider-routing-configuration.md) および [SOT-IF-040](../sot/40-interfaces/40-judicial-cases-pack-activation.md) のうち、既存設定項目、`providers` と `providerRoutes`、key 単位の atomic な上書き、明示的な空 namespace、credential 環境変数参照の構造検証、設定ファイルだけから読み込む既定無効の `extensionPacks.judicial-cases.enabled`、一括構成が未完了な間の transport 起動前 fail-closed、および有効な組込み provider と能力別 route の起動前照合
...
- [SOT-IF-047](../sot/40-interfaces/47-mcp-search-judicial-cases.md) に従う、固定された収録範囲の注意、共通モデルの `ref`・provenance・page を変更しない出力、正確な整数入力、空結果および公開情報源エラーを扱う package 内の `search_judicial_cases` MCP handler と契約テスト
...
- [SOT-IF-040](../sot/40-interfaces/40-judicial-cases-pack-activation.md) の公開面を含む一括構成、[SOT-IF-048](../sot/40-interfaces/48-mcp-get-judicial-case.md) の `get_judicial_case`、および二つの裁判例 MCP ツールを原子的に composition root へ接続する処理は未実装である。`search_judicial_cases` の handler は実装済みだが、単独では production server へ登録しない。
...
無設定時は e-Gov 法令 API Version 2 の四つと、e-Gov 法令 API Version 1 の `law.update.list@1` の、合計五つの組込み primary route を起動時に構成する。`judicial-cases` が有効な場合は `courts-hanrei-html` の二つも内部 registry と route へ構成するが、二つの MCP ツールを一括公開できるまでは transport 開始前に fail-closed する。公開 MCP ツールは従来の五つである。利用者は設定ファイルで provider の有効化、能力別 primary route および rollback override を指定でき、無効な組合せは transport の開始前に終了コード `2` で拒否する。組込み provider は provider-specific setting と credential slot を持たず、非空の `settings` と `credentialEnvRefs` は受け付けない。
