...
| 6 | 一部完了 | 6.1 の `GET /laws` は完了し、public facade と capability の共有 parser、runtime 応答と保存済み契約の分類分離、および page invariant を実装した。共通 `lawtarget` resolver と `search_laws`／統合照会 law search facade の page 内安定優先も実装済みで、残りは 6.2 `GET /keyword` と 6.3 `GET /law_data` の段階移行である | `SOT-IF-011`、`SOT-IF-052`、`SOT-IF-053`、`SOT-IF-054`、`SOT-ARCH-030` |
