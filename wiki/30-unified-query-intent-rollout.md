...
構成が同じ観測を返すことを固定した。独立 review は通過し、今回の commit に
対する権威 CI の判定対象である。
...
| 4.3 | 実装・独立 review 通過（権威 CI 判定対象） | holdout を使わない development 専用 loader、原 byte digest、次版 profile set 校正および四十三件全体の決定的 fingerprint |
