...
func newPublicDependencies(
	...
	judicialCitations, err := newJudicialCitationsDependencies(cfg, registry, routes)
	if err != nil {
		return projectmcp.Dependencies{}, err
	}
	...
	return projectmcp.Dependencies{
		...
		JudicialCitations:     judicialCitations,
...
func newEnabledProviderBindings(
	...
	for _, providerID := range providerIDs {
		...
		case hanreipdf.Descriptor().ProviderID():
			binding, err = hanreipdf.NewProviderBindings()
