...
func loadSchemas(root *legalqueryartifact.Repository) (artifactSchemas, error) {
	...
	for _, schema := range []struct {
		...
		if err != nil {
			return artifactSchemas{}, fmt.Errorf("candidate evaluation %s を読めません: %w", schema.name, err)
		}
		if !bytes.Equal(raw, schema.raw) {
			return artifactSchemas{}, fmt.Errorf("candidate evaluation %s が固定済み schema と一致しません", schema.name)
		}
	}
	return loadCanonicalArtifactSchemas()
}
