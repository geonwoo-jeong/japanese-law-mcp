...
func TestDescriptorMatchesSOTIF072(t *testing.T) {
	t.Parallel()
	...
	if descriptor.ProviderID() != providerID ||
		...
		descriptor.AdapterContractVersion() != "1.2.0" ||
		descriptor.VerifiedAt().String() != "2026-07-26" ||
		descriptor.InterfaceType() != model.InterfaceTypeHTML ||
		descriptor.CredentialRequired() {
		t.Errorf("SOT-IF-072: descriptor の固定値が一致しない")
	}
	capabilities := descriptor.Capabilities()
	if len(capabilities) != 3 ||
		capabilities[0].ID() != "judicial-decision.citing-candidate.search" ||
		capabilities[1].ID() != "judicial-decision.read" ||
		capabilities[2].ID() != "judicial-decision.search" {
		t.Fatalf("SOT-IF-072: capabilities = %#v", capabilities)
	}
	for _, capability := range capabilities {
		if capability.MajorVersion() != 1 ||
			...
			t.Errorf("SOT-IF-072: capability = %#v", capability)
		}
	}
}
