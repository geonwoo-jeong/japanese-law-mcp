...
func TestParseLawSearchResponseClassifiesStructuralError(t *testing.T) {
	...
	assertLawSearchSourceError(t, err, model.SourceErrorCodeSourceContractChanged)
	...
	tests := []struct {
		...
		},
		{
			...
			body: `{"total_count":1,"count":1,"laws":[{"law_info":{"law_id":"322CO0000000016","promulgation_date":null},"revision_info":{"law_revision_id":"322CO0000000016_20240401_506CO0000000161","law_title":"地方自治法施行令"}}]}`,
			code: model.SourceErrorCodeSourceContractChanged,
		},
	}
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			_, _, parseErr := parseLawSearchResponse(
				context.Background(),
				[]byte(test.body),
				1,
				0,
