...
const repositoryCorpusV15DerivedObservationPopulationVerificationID = "legal-query-corpus-derived-observation-population"

func TestRepositoryCorpusV15は全派生観測の母集団を持つ(t *testing.T) {
	repositoryRoot := repositoryCorpusV15Root(t)
	...
	if err != nil {
		t.Fatalf(
			"%s: corpus-v15 を読めません: %v",
			repositoryCorpusV15DerivedObservationPopulationVerificationID,
			err,
		)
	}
...
func repositoryCorpusV15Root(t *testing.T) string {
	t.Helper()
