...
func Test組込み辞書とCorpusV2の法概念IDは相互に対応する(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("SOT-ENG-023, SOT-ENG-026: corpus-v2 Load() error = %v", err)
	}
	...
	for conceptID := range knownConceptIDs {
		...
		t.Errorf(
			"SOT-ENG-023: 組込み辞書の conceptId %q に corpus-v2 の fixture がありません",
			conceptID,
		)
	}
}
